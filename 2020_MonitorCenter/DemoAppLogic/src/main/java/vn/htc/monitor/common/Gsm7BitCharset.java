    ...
    private class Gsm7BitEncoder extends CharsetEncoder {
        ...
        protected CoderResult encodeLoop(CharBuffer cb, ByteBuffer bb) {
            ...
            while (cb.hasRemaining() && bb.hasRemaining()) {
                ...
                if (debug) {
                    MyLog.logDebug("Encoding ch " + ch + " to byte " + b);
                }
                if (b != null) {
                    ...
                    if (debug) {
                        MyLog.logDebug("Trying extended map to encode ch " + ch + " to byte " + b);
                    }
                    if (b != null) {
    ...
    private class Gsm7BitDecoder extends CharsetDecoder {
        ...
        protected CoderResult decodeLoop(ByteBuffer bb, CharBuffer cb) {
            ...
            while (bb.hasRemaining() && cb.hasRemaining()) {
                ...
                // first check the default alphabet
                if (debug) {
                    MyLog.logDebug("Looking up byte " + b);
                }
                ...
                if (s != null) {
                    ...
                    if (ch != '\u001B') {
                        if (debug) {
                            MyLog.logDebug("Found string " + s);
                        }
                        cb.put(ch);
                    } else {
                        if (debug) {
                            MyLog.logDebug("Found escape character");
                        }
                        ...
                        if (bb.hasRemaining()) {
                            ...
                            if (s != null) {
                                if (debug) {
                                    MyLog.logDebug("Found extended string " + s);
                                }
                                ch = s.charAt(0);
                                cb.put(ch);
                            } else {
