var app = angular.module('MessageMonitorApp', ['ui.bootstrap']);
app.controller('messageMonitorCtrl', function ($scope, $http, $filter) {
    $scope.reloadFilter = function (str) {
        $.blockUI({message: '<img src="' + context + '/admin/images/indicator_48.gif" />'});
        $http({
            method: "POST",
            url: urlBase + "/admin/message-monitor/rest/recycle-data",
            params: {crPage: $scope.crPage, key: $scope.key, phone: $scope.ip, sento: $scope.sento, status: $scope.status, maxRow: $scope.maxRow}
