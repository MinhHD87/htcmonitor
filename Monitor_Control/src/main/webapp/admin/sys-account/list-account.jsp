                        ...
                        <form class="form-horizontal">
                            ...
                            <div class="control-group">
                                <div class="controls">
                                    ...
                                    &nbsp;&nbsp;&nbsp;
                                    <input ng-if="roles.add" class="btn btn-primary" onclick="location.href = '<c:url value="/admin/sys-account/create" />'" value="${Lang['generic.addNew']}" type="button">
                                </div>
                            </div>
                        </form>
                    </div>
                ...
                <div class="widget-box">
                    ...
                    <div class="widget-content nopadding">
                        <table class="table table-bordered table-striped">
                            <thead>
                                <tr>
                                    <th>${Lang['generic.index']}</th><th>${Lang['generic.userName']}</th><th>${Lang['generic.fullName']}</th><th>${Lang['generic.desc']}</th><th>${Lang['generic.phone']}</th>
                                    <th>${Lang['generic.email']}</th><th>${Lang['generic.createDate']}</th><th>${Lang['generic.createBy']}</th>
                                    <th ng-if="roles.confRole">${Lang['generic.role']}</th><th>${Lang['generic.status']}</th><th ng-if="roles.edit || roles.del">${Lang['generic.edit']}/${Lang['generic.delete']}</th>
                                    <th ng-if="roles.viewFull">${Lang['generic.detail']}</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr ng-repeat="oneAcc in datas">
                                    ...
                                    <td>{{oneAcc.phone}}</td>
                                    <td>{{oneAcc.email}}</td>
                                    <td ng-bind="formatDateTime(oneAcc.createDate)"></td>
                                    <td>{{oneAcc.createBy}}</td>
                                    <td ng-if="roles.confRole" class="img-center"><a href="${pageContext.request.contextPath}/admin/sys-account/user-privileges?id={{oneAcc.accId}}">${Lang['generic.role']}</a></td>
                                    <td class="img-center">
                                    ...
                                    <td ng-if="roles.edit || roles.del" style="width: 70px">
                                        <a ng-if="roles.edit" href="${pageContext.request.contextPath}/admin/sys-account/edit?id={{oneAcc.accId}}" >
                                            <img src="<c:url value="/admin/images/edit.png"/>" title="Edit">
                                        </a>
                                        <a ng-if="roles.del" ng-click="delete(oneAcc.accId)" href="" title="Delete">
                                            <img src="<c:url value="/admin/images/remove.png"/>" title="Delete">
                                        </a>
                                    </td>
                                    <td ng-if="roles.viewFull">
                                        <a ng-click="detail(oneAcc.accId)" href="" title="Chi tiết">
